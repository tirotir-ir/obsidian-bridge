...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('obsidianBridge.sendSelectionToNote', sendSelectionToNote), vscode.commands.registerCommand('obsidianBridge.appendSelectionToCurrentNote', appendSelectionToCurrentNote), vscode.commands.registerCommand('obsidianBridge.sendSelectionToCanvas', sendSelectionToCanvas), vscode.commands.registerCommand('obsidianBridge.sendCurrentMarkdownFile', sendCurrentMarkdownFile), vscode.commands.registerCommand('obsidianBridge.configureVault', configureVault), vscode.commands.registerCommand('obsidianBridge.configureCanvas', configureCanvas));
}
function deactivate() {
    output?.dispose();
}
function getConfig() {
    ...
    return {
        vaultPath: config.get('vaultPath', '').trim(),
        canvasPath: config.get('canvasPath', '').trim(),
        defaultFolder: config.get('defaultFolder', 'Inbox').trim(),
        vaultName: config.get('vaultName', '').trim(),
        companionServerUrl: config.get('companionServerUrl', 'http://127.0.0.1:27124').trim().replace(/\/$/, ''),
        companionToken: config.get('companionToken', '').trim(),
        currentFileMode: config.get('currentFileMode', 'ask'),
    };
}
async function requireVault() {
    const config = getConfig();
    if (!config.vaultPath) {
        const configure = await vscode.window.showErrorMessage('Obsidian Bridge: Set obsidianBridge.vaultPath before sending files.', 'Configure Vault Path');
...
async function ensureFolder(vaultPath, relativeFolder) {
...
function vaultRelative(vaultPath, absolutePath) {
...
async function openInObsidian(vaultPath, absolutePath, configuredVaultName) {
...
async function sendSelectionToNote() {
...
async function appendSelectionToCurrentNote() {
    const text = selectionOrThrow();
    if (!text) {
        return;
    }
...
async function chooseVaultMarkdownFile(config, title) {
...
async function sendSelectionToCanvas() {
    const text = selectionOrThrow();
    if (!text) {
        return;
    }
    ...
    try {
        ...
        try {
            ...
            const { canvas, node } = (0, canvas_1.appendTextNode)(raw, text);
            await fs.writeFile(canvasPath, (0, canvas_1.serializeCanvas)(canvas), 'utf8');
            ...
            if (isInvalidCanvasError(error)) {
                const action = await vscode.window.showErrorMessage(`Obsidian Bridge: ${errorMessage(error)} Select a valid .canvas file from the configured Vault.`, 'Choose Another Canvas');
...
function isInvalidCanvasError(error) {
...
function resolveConfiguredCanvas(config) {
...
async function ensureCanvasConfigured(config) {
    ...
    if (configured) {
        ...
        const choose = await vscode.window.showWarningMessage(`Obsidian Bridge: Canvas path is missing or invalid: ${config.canvasPath}`, 'Choose Canvas');
...
async function chooseCanvasFile(config) {
    ...
    try {
        ...
        await vscode.workspace.getConfiguration('obsidianBridge').update('canvasPath', relative, vscode.ConfigurationTarget.Global);
...
async function sendCurrentMarkdownFile() {
    const editor = vscode.window.activeTextEditor;
    ...
    try {
        ...
        if (mode === 'link') {
            try {
                ...
                const choice = await vscode.window.showWarningMessage(`Obsidian Bridge: Could not create a hard link (${errorMessage(error)}). Copy the file instead?`, 'Copy Instead');
...
async function chooseTransferMode(configuredMode) {
    if (configuredMode !== 'ask') {
        return configuredMode;
    }
    const picked = await vscode.window.showQuickPick([
        { label: 'Copy', description: 'Create an independent copy in the vault.', mode: 'copy' },
        { label: 'Hard link', description: 'Share file contents when both locations are on the same Windows drive.', mode: 'link' },
    ], {
...
function resolveVaultPath(vaultPath, suppliedPath) {
...
async function getCompanionPath(config, endpoint) {
...
async function postToCompanion(config, endpoint, body) {
    if (!config.companionToken) {
        return undefined;
    }
    try {
